...
srb_basal_ab = ['CGTACTAG-TATCCTCT','CGTACTAG-GTAAGGAG','PB50','AGGCAGAA-TATCCTCT','GTAGAGGA-TAGATCGC','TAAGGCGA-CTAAGCCT','PB82','PB90','PB27','AGGCAGAA-GTAAGGAG','CGTACTAG-CTAAGCCT','TAAGGCGA-TATCCTCT','PB18','CGTACTAG-AAGGAGTA','GTAGAGGA-AGAGTAGA','PB10','PB34','PB76']
...
mask_f = [True if x in srb_f_clade else False for x in data_index]
mask_ab = [True if x in srb_basal_ab else False for x in data_index]
mask_top_ab = [True if x in srb_top_ab_clade else False for x in data_index]
...
for i in range(0,len(data_positions)):
	...
	if sum(p==1)/(len(srb_f_clade)) >= 0.5:
		...
		p = np.array(data[data_positions[i]].tolist())[mask_ab]
		if sum(p==1)/(len(srb_basal_ab)) >= 0.5:
			fixed_loci.append(data_positions[i])
			clade.append('AB')
			chr.append(data.iloc[0,i])
		else:
			p = np.array(data[data_positions[i]].tolist())[mask_top_ab]
			if sum(p==1)/(len(srb_top_ab_clade)) >= 0.5:
				fixed_loci.append(data_positions[i])
				clade.append('topAB')
				chr.append(data.iloc[0,i])
...
file = '/home/ada/Desktop/Shraiman_lab/srb_data/snp_matrix_fixed_F_basalAB_topAB_clade_threshold50_NaNs30wholegenome_fixed.csv'
snp_df.to_csv(file)