"""
Make a histogram of lengths for green and yellow haplotypes. 
"""
...
lengths_file_path = '/home/ada/Desktop/Shraiman_lab/srb/haplotyping/haplotypes_lengths.txt'
...
with open(lengths_file_path, 'r') as f:
    for line in f:
        if line.startswith('#'):
            ...
            green_lengths = f.readline().split('=')[1].replace('[','').replace(']','') # get rid of any unnecessary characters
            ...
            # same for the yellow lengths data
            yellow_lengths = f.readline().split('=')[1].replace('[','').replace(']','')
            yellow_lengths = [int(i) for i in yellow_lengths.split(',')]
            yellow_lengths = [i for i in yellow_lengths if i != 0]


            data_dict[condition] = [green_lengths, yellow_lengths]
...
for condition in data_dict.keys():
    sizes, bins, _ = plt.hist(data_dict[condition][0], bins=bins)
