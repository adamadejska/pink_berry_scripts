###########################################################################
# Haplotyping strains from the mixing layer in PSB.
# This script attempts to haplotype the strains from the mixing layer based 
# on three haplotype profiles: 9 clade (G), missing,  and wild type (WT).
...
def make_all_profiles(clade_9_strains, snp_matrix):

    # Make profile probability matrices for each haplotype
    green_prob_matrix = make_profile_matrix(clade_9_strains, snp_matrix)
    white_prob_matrix = make_type_profile_matrix(clade_9_strains, snp_matrix, 'WT')  # strain input doesn't really matter here.
    missing_prob_matrix = make_type_profile_matrix(clade_9_strains, snp_matrix, 'M')

    return([green_prob_matrix, white_prob_matrix, missing_prob_matrix])


def run_haplotyping(start, length, window_size, strain, snp_matrix, max_penalty, profiles):
    ...
    plt.legend()
    plt.title(strain + ' positions ' + str(columns[0]) + ' - ' + str(columns[-1]) + 'bp log likelihood graph for haplotypes')
    plt.tight_layout()
    plt.grid()
    #plt.show()
    ...
    plt.title(strain + ' positions ' + str(columns[0]) + ' - ' + str(columns[-1]) +  'bp  B-F haplotype sequence')
    #plt.show()
    return(haplotype)
...
def get_haplotype_lengths(haplotype, positions):
    ...
    for i in range(0, len(green_hap_only)):
        if green_hap_only[i] == 1:
            ...
            start = str(green_hap_only[i]).split('.')[0]    # treat them as a block of length 2 instead of 1 
...
if __name__ == "__main__":
    ...
    profiles = make_all_profiles(clade_9_strains, snp_matrix)
