...
clades = [e_clade, clade_9_strains, ladder, basal_clade]
...
plt.plot(avg_singletons, a*avg_singletons+b, color='r')

plt.scatter(avg_singletons, avg_counts, color='black')
plt.title('What is the rate of transfer of fixed SNPs from one clade to other clades?\n BC clade PSB + best fit line')
plt.xlabel('average number of singletons per clade')
plt.ylabel('average number of SNPs transferred')
plt.show()
...
plt.plot(avg_singletons, func(avg_singletons, popt),color='r')


plt.scatter(avg_singletons, avg_counts, color='black')
plt.title('What is the rate of transfer of fixed SNPs from one clade to other clades?\n BC clade PSB + best fit line through origin')
plt.xlabel('average number of singletons per clade')
plt.ylabel('average number of SNPs transferred')
plt.show()
...
p = np.polyfit(avg_singletons[1:], np.log(avg_counts[1:]), 1,w=np.sqrt(avg_counts[1:]))

# Convert the polynomial back into an exponential
a = np.exp(p[1])
b = p[0]
x_fitted = np.linspace(np.min(avg_singletons[1:]), np.max(avg_singletons[1:]), 100)
y_fitted = a * np.exp(b * x_fitted)

plt.plot(x_fitted, y_fitted, color='r')

plt.scatter(avg_singletons[1:], avg_counts[1:], color='black')
plt.title('What is the rate of transfer of fixed SNPs from one clade to other clades?\n 8 clade PSB + best fit line weighted exponential')
plt.xlabel('average number of singletons per clade')
plt.ylabel('average number of SNPs transferred')
plt.show()
