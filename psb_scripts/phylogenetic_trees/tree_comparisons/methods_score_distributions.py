...
def roundup(n):
    ...
    return(round_num)
"""
data = pd.read_csv("/home/ada/Desktop/PinkBerry_scripts_paper/psb_scripts/phylogenetic_trees/tree_comparisons/Visual TreeCmp Comparison of Phylogenetic Trees on the Web - Report.csv", index_col=0)
...
for i in methods:
    ...
    if max_distance > 60:
       bin_step = roundup(int(max(np.array(data.loc[:, methods[0]]))//20))
    else:
        bin_step = 1

    bins = range(0, roundup(max_distance), bin_step)
    #plt.xticks(bins)
...
data = pd.read_csv("/home/ada/Desktop/PinkBerry_scripts_paper/psb_scripts/phylogenetic_trees/tree_comparisons/Visual TreeCmp Comparison of Phylogenetic Trees on the Web - Report-More_complex.csv", index_col=0)
