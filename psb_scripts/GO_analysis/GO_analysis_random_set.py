...
# Genes in the blocks
#psb_snp_dense_genes_path = '/Volumes/Data/Shraiman_lab/GO_analysis/PSB_genes_in_blocks.csv'
...
low_n_codes = []
exp_code= []
for k,v in dense_bp.items():
    if v >= 1:   # Get rid of single hits for graph readability
        exp_code.append(k)

for code in exp_code:    # Use only the codes that were found in experimental, we don't care about others
    if bio_processes_all[code] <= 2:
...
for id in dense_ids:
    for code in low_n_codes:
        if code in id_to_GO_codes[id][0][0]:   # Check only biological processes
            low_n_codes_ids.append(id)

print(low_n_codes_ids)
"""
# Find genes with specified codes. 
specific_codes = ["0051301", "0000160", "0007059", "0006313"]
...
for id in dense_ids:
    for code in specific_codes:
        if code in id_to_GO_codes[id][0][0]:   # Check only biological processes
            specific_codes_ids.append(id)

print(specific_codes_ids)
"""