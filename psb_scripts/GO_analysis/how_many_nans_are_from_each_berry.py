...
gene_codes = ['rhsC_5']
gene_names = ['Putative deoxyribonuclease RhsC ( rhsC_5 )']
...
for n in range(0,len(gene_codes)):
        ...
        if int(i) >= start and int(i) <= end:
            ...
            for j in range(0, len(data_index)):
                if not np.isnan(tmp[j]):
                    ...
                    berry_specific_nans[data_index[j]] += 1
    ...
    # Take only the berries that have nans. We don't want to plot a bunch of zeros.
    num_of_nans = []
    for k in valid_berries:
        num_of_nans.append(berry_specific_nans[k])
    ...
    plt.scatter(num_of_mutations, num_of_nans, color=colors)
    plt.title(gene_names[n] + '\n gene length: ' + str(end-start))
    plt.xlabel('Number of mutations in a gene per berry')
    plt.ylabel('Number of missing data in a gene per berry')
    plt.grid()
    plt.show()
