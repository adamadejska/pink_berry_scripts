...
plt.title('Distribution of number of mutations per XerD gene copy')
plt.show()
"""
    ...
    for line in f:
        if line.startswith('psb-scaff03'):
                if gene_code in line:
                    ...
                    plt.title('Tyrosine recombinase Xer ' + str(start) + ' - ' + str(end) + '\n number of mutations: ' + str(mutation_counter))
...
plt.barh(valid_berries, num_of_mutations)
plt.title('Tyrosine recombinase XerD \n all copies together' )
plt.xlabel('Number of mutations per berry')
plt.ylabel('Berry names')
plt.grid(axis = 'x')
plt.show()
