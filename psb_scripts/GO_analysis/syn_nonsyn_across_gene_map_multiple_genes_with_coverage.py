...
current_genes = genes_wapA2
current_names = gene_names_wapA2
title = 'wapA_2 and the hypothetical proteins proceeding it'
...
color = 'tab:red'
ax1.set_ylabel('Average coverage', color=color)
ax1.plot(positions, coverage, color=color)
...
ax2.scatter(snp_positions, nonsyn, color='green', alpha=0.8, label='nonsynonymous')
ax2.scatter(snp_positions, syn, color='red', alpha=0.8, label='synonymous')
ax2.scatter(snp_positions, noncoding, color='gray', alpha=0.8, label='noncoding')

# Color the area of each ORF on the graph
for orf in current_genes:
    ax2.axvspan(orf[0], orf[1], facecolor='gray', alpha=0.3)
    ax2.axvline(orf[0], color = 'gray', alpha=0.4)
    ax2.axvline(orf[1], color = 'gray', alpha=0.4)

ax2.set_xticks(range(snp_positions[0], snp_positions[-1], 1000))
